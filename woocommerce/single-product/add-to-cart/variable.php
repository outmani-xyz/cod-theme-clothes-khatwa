 ...
 * Engineering refinement for khutwa
 */
...
<form class="variations_form cart" action="<?php echo esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())); ?>" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint($product->get_id()); ?>" data-product_variations="<?php echo $variations_attr; ?>">
    ...
    <?php if (empty($available_variations) && false !== $available_variations) : ?>
        <p class="stock out-of-stock"><?php echo esc_html(apply_filters('woocommerce_out_of_stock_message', __('This product is currently out of stock.', 'khutwa'))); ?></p>
    <?php else : ?>
                    ...
                    <tr>
                        ...
                        <td class="value">
                            ...
                            if (taxonomy_exists($attribute_name) && strpos($attribute_name, 'size') !== false) {
                                // Buttons
                                echo '<div class="custom-swatches d-flex flex-wrap gap-2 mb-2" data-attribute_name="attribute_' . esc_attr($attribute_name) . '">';
                                foreach ($options as $option) {
                                    $term = get_term_by('slug', $option, $attribute_name);
                                    $label = $term ? $term->name : $option;
                                    echo '<button type="button" class="swatch-btn btn btn-outline-dark" data-value="' . esc_attr($option) . '">' . esc_html($label) . '</button>';
                                }
                                echo '</div>';
